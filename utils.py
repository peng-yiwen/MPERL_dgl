...
def load_graph(data_dir, e2id, r2id, t2id, loadET=True, loadKG=True):
    ...
    return g, train_label, valid_label, all_true, train_id, valid_id, test_id
...
def evaluate(path, predict, all_true, e2id, t2id):
    ...
    with open(path, 'r', encoding='utf-8') as r:
        for line in r:
            e, t = line.strip().split('\t')
            e, t = e2id[e], t2id[t]
            scale = predict[e].max() - predict[e].min() + 1 # add 1, making sure superior than max-min
            tmp = predict[e] - all_true[e] * scale
            tmp[t] = predict[e, t]
